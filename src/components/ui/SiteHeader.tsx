    ...
    <>
      <header className="fixed top-0 z-[70] w-full border-b border-on-surface/10 bg-background-paper/90 backdrop-blur-md no-print">
        <div className="mx-auto flex h-20 max-w-container-max items-center justify-between px-margin-mobile md:px-margin-desktop">
          <AppLink to="/" className="flex shrink-0 items-center" onClick={() => setMenuOpen(false)}>
            <img
              alt="STOP&SCAN logo"
          ...
          <nav className="hidden items-center gap-xl md:flex">
            {NAV_LINKS.map(({ to, label }) => (
              <AppNavLink key={to} to={to} currentPath={currentPath}>
                {label}
              </AppNavLink>
            ))}
          </nav>
...
function MobileMenuOverlay({
  ...
  return (
    ...
    >
      ...
      <nav className="flex flex-1 flex-col justify-center gap-sm px-margin-mobile py-xl">
        {NAV_LINKS.map(({ to, label }, index) => (
          <AppNavLink
            ...
            className={`${open ? "translate-y-0 opacity-100" : "translate-y-3 opacity-0"}`}
          >
            {label}
          </AppNavLink>
        ))}
      ...
      >
        <AppLink to="/journal" onClick={onClose} className="btn-primary flex w-full justify-center py-md text-center">
          My Journal
        </AppLink>
      </div>
    </div>
