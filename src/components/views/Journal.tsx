    ...
    <div className="px-margin-mobile pb-xxl pt-8 md:px-margin-desktop">
      <div className="mx-auto max-w-2xl">
        <div className="mb-4 flex items-center justify-between no-print">
          <AppLink to="/journal" className="font-label-md text-on-surface-variant hover:text-on-surface">
            ← All reflections
          </AppLink>
...
export default function Journal({ entryId: entryIdProp }: { entryId?: string }) {
  const [entryId, setEntryId] = useState(entryIdProp);

  useEffect(() => {
    if (entryIdProp) {
      setEntryId(entryIdProp);
      return;
    }
    const params = new URLSearchParams(window.location.search);
    const fromQuery = params.get("entry");
    if (fromQuery) setEntryId(fromQuery);
  }, [entryIdProp]);
      ...
      <div className="mx-auto max-w-container-max">
        ...
        {entries.length === 0 ? (
          <div className="flex flex-col items-center gap-lg py-xxl text-center">
            ...
            <div className="flex gap-md">
              <AppLink to="/learn" className="btn-primary">
                Start the lesson
              </AppLink>
